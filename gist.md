...
**Whole-project versioning is a first-class primitive.** Checkpoints atomically snapshot every git repo involved, the Beads database, live session context summaries, sidecar state, and artifacts. Operators can "undo back to yesterday" and roll every dimension back together.

**Bootstrap** pulls from Jira or Beads — a doc-import adaptor reads existing epics, stories, and specs; a PM + Documentarian + Architect trio generates the project description, initial goals, candidate values, and starting plan; the operator ratifies. Workspace repos (the `gemba_prime` / `gemba` pattern) are supported first-class.
...
**Extensibility: adaptors, personas, packs.** Write a WorkPlane or OrchestrationPlane adaptor against the typed contract and run the importable conformance harness. Curate a persona, or a role pack bundling personas + skills + default values — signed for provenance, public or private.
