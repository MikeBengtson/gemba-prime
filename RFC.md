...
### 2. Scrum / day-of ops — the Gemba is the home screen

The Gemba — an `AgentGroup`-grouped Epic-granular Kanban — is what opens on `gemba serve` — drag-drop status changes round-trip through `WorkPlaneAdaptor.transition`; multi-select cards to dispatch via `OrchestrationPlaneAdaptor.acquire_workspace + start_session`.
